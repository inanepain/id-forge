...
- `random(int $length): string` — generates a cryptographically secure random string using the active alphabet

Concrete encoders (Base32/58/64) extend this class.

.Generate a random string with the encoder's alphabet
[source,php]
----
use Inane\IdForge\EncoderFactory;

$base58 = EncoderFactory::createBase58();
$token  = $base58->random(22); // 22-char Base58 string
----
...
- Errors: throws `InvalidArgumentException` if a character isn’t in the alphabet during `decode()`
...
- Methods:
  ...
  - `generate(): string`
  - `toEncoded(EncoderInterface $encoder): string`
- Behaviour: on sequence overflow within the same millisecond, waits for the next millisecond
...
- Most decoders will throw `Inane\Stdlib\Exception\InvalidArgumentException` when the input contains invalid characters or can’t be parsed.
...
- Generators that rely on randomness may throw `Random\RandomException` from PHP core when entropy isn’t available.
